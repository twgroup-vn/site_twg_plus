...
var Zoomable = function () {
    function Zoomable(zoomableEls) {
        ...
        /*
            ## Documentation
            'zoomableEls' should be a DOM element, or an array of DOM elements
             These DOM elements can be either:
            * <a>
                * Image URL: any address ending with .gif / .jpg / .jpeg / .png
                * YouTube video URL: either in the long form (https://www.youtube.com/watch?v=CMJWtufWQp4)
                  or short form (https://youtu.be/CMJWtufWQp4)
            * <img>
                * Any URL according to the same rules as above
                * Optionally, a bigger image URL can be passed to a 'data-zoomable-full-size' attribute
                  to override the default 'src' attribute when zooming in.
            * <video>
                * Any URL pointing to a video file
            * Any other element: In theory, as of right now, if one of the 'zoomableEls' is a <p> it
              will zoom in on that same <p> element. This behavior is not optimal. In the future, it
              should also work with any hidden element, and this will be the 'backdoor' for it.
              This will be tweaked in the future to prevent misuses.
             ## ToDo:
            *
             ## Not necessary for now:
            * Zoom on a hidden element (iframe / p)
            * Load async content
        */
        ...
        this.elements = {
            ...
            wrapper: $$('.td-zoomable')[0],
            contentHolder: $$('.td-zoomable__content_holder')[0],
            loader: $$('.td-zoomable__loader')[0],
            close: $$('.td-zoomable__close')[0],
            background: $$('.td-zoomable__background')[0]
        };
        ...
        try {
            for (var _iterator = this.elements.triggers[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                var image = _step.value;

                image.addEventListener('click', function (event) {
                    $preventDefault(event);
                    _this.setContent(event.target);
                }, false);
            }
        ...
        this.elements.background.addEventListener('click', function () {
            _this.hide();
        });
    ...
    _createClass(Zoomable, [{
        key: 'setHTMLinPlace',
        value: function setHTMLinPlace() {
            $add($body(), DIV({ c: 'td-zoomable td-zoomable--hidden' }, DIV({ c: 'td-zoomable__background' }), DIV({ c: 'td-zoomable__content' }, DIV({ 'c': 'td-zoomable__close' }, DIV({ 'c': 'td-zoomable__close_icon' })), DIV({ 'c': 'td-zoomable__loader td-zoomable--hidden' }, Indicator.cssSpinner()), DIV({ c: 'td-zoomable__content_ratio' }), DIV({ c: 'td-zoomable__content_scroll' }, DIV({ c: 'td-zoomable__content_holder' })))));
        }
    }, {
        key: 'setContent',
        value: function setContent(targetEl) {
            var _this2 = this;

            var content = void 0;

            if ($nodeName(targetEl) == "a") {
                // It's an <a>
                var hrefValue = targetEl.getAttribute('href');

                if (this.isYouTubeVideo(hrefValue)) {
                    // YouTube video

                    content = this.getYouTubeVideoContent(hrefValue);

                    $addClass(this.elements.contentHolder, 'td-zoomable__content_holder--no_padding');
                    $addClass(this.elements.wrapper, 'td-zoomable--16by9');
                } else if (this.isImageURL(hrefValue)) {
                    // http://todoist.com/logo.png

                    content = IMG({ src: hrefValue });
                }
            } else {
                // It's another element
                var fullSizeImg = targetEl.getAttribute('data-zoomable-full-size');

                content = targetEl.cloneNode(true);

                if (fullSizeImg) {
                    content.setAttribute('src', fullSizeImg);
                }

                $removeClass(content, 'td-zoomable__item');
            }

            if (content) {
                $replace(this.elements.contentHolder, content);

                ['load', 'loadeddata'].forEach(function (event) {
                    content.addEventListener(event, function () {
                        $addClass(_this2.elements.loader, 'td-zoomable--hidden');
                    });
                });
        ...
        key: 'getYouTubeVideoContent',
        value: function getYouTubeVideoContent(videoURL) {
            var videoURLMatch = this.isYouTubeVideo(videoURL, true);
            if (videoURLMatch) {
                return IFRAME({ src: 'https://www.youtube.com/embed/' + videoURLMatch[2] + '?rel=0&amp;showinfo=0', frameborder: '0', allowfullscreen: true });
            }
            return false;
        }
    }, {
        key: 'show',
        value: function show() {
            $removeClass(this.elements.wrapper, 'td-zoomable--hidden');
            $setStyle($body(), {
                overflow: 'hidden'
            });
        }
        ...
        value: function hide() {
            ...
            $addClass(this.elements.wrapper, 'td-zoomable--hidden');
            $setStyle($body(), {
                overflow: 'visible'
            });
        }
        ...
        key: 'isYouTubeVideo',
        value: function isYouTubeVideo(URL, returnMatch) {
            var youTubeVideoRegex = /https?:\/\/(?:www.)?(youtu\.be|youtube.com)\/(?:watch\?v=)?([\w-]+)/;
            return returnMatch ? URL.match(youTubeVideoRegex) : youTubeVideoRegex.test(URL);
        }
    }, {
        key: 'isImageURL',
        value: function isImageURL(string) {
            return (/(\.gif|\.jpg|\.jpeg|\.png)$/.test(string)
            );
        }
    }, {
        key: 'setDefaults',
        value: function setDefaults() {
            $removeClass(this.elements.contentHolder, 'td-zoomable__content_holder--no_padding');
            $removeClass(this.elements.wrapper, 'td-zoomable--16by9');
            $removeClass(this.elements.loader, 'td-zoomable--hidden');
            $replace(this.elements.contentHolder, '');
        }
    }]);
...
var _es6Zoomable = require('./.es6.Zoomable.js');

var _es6Zoomable2 = _interopRequireDefault(_es6Zoomable);
...
$AEV(window, 'load', function () {
    new GuideTOC($$(".td-guide__sidebar")[0]);
    new _es6Zoomable2.default($$(".js-zoomable-item"));
});

var GuideTOC = function () {
    function GuideTOC(wrapperEl) {
        var _this = this;

        _classCallCheck(this, GuideTOC);

        if (!wrapperEl) {
            console.error("GuideTOC module requires an element as argument!");
            return false;
        }

        this.elements = {
            toc: wrapperEl,
            tocInner: $$(".td-guide__sidebar-inner")[0],
            tocItems: $$(".td-guide__toc-link"),
            guide: $$(".td-guide__main")[0],
            progressBars: $$(".td-guide__progress-bar"),
            mobileNav: $$(".td-guide__mobile-nav")[0],
            mobileNavTree: $$(".td-guide__mobile-nav-tree")[0],
            mobileNavTitle: $$(".td-guide__active-section-title")[0],
            feedbackWrapper: $$(".td-guide__feedback")[0],
            weeklyTaskCTA: $$("#add-weekly-task")[0],
            weeklyTaskNotification: $$("#weekly-task-ajax-notification")[0],
            dailyTaskCTA: $$("#add-daily-task")[0],
            dailyTaskNotification: $$("#daily-task-ajax-notification")[0],
            feedbackInputs: $$(".td-guide__feedback-input"),
            feedbackSubmitBtn: $$(".td-guide__feedback-submit-button")[0],
            feedbackTextbox: $$("#feedback-comments")[0],
            feedbackNotification: $$("#feedback-ajax-notification")[0],
            feedbackCommentNotification: $$("#feedback-comment-ajax-notification")[0],
            guideWrapper: $$(".td-guide")[0]
        };
        ...
        this.feedbackSelectedItem = null;

        this.messages = {
            error: _('An error occurred, please try again later.'),
            taskAdded: _('Task has been added')
        };
        ...
        window.addEventListener("load", function () {
            ...
            _this.doWhenResizing();

            if (_this.guideIsMinimal(_this.elements.guideWrapper)) {
                _this.unwrappLinks($$('a', _this.elements.guide));
            }
        }, false);

        // Scroll
        document.addEventListener("scroll", function () {
            _this.throttle(_this.doWhenScrolling, _this.baseFps);
        }, false);

        // Resize
        window.addEventListener("resize", function () {
            _this.throttle(_this.doWhenResizing, _this.baseFps);
        }, false);

        // Mobile nav change
        this.elements.mobileNavTree.addEventListener("change", function (event) {
            _this.goToSection(event, event.target);
        }, false);

        // Hash change
        // Used to correct offset of mobileNav element
        window.addEventListener("hashchange", function () {
            window.scrollTo(window.scrollX, window.scrollY - _this.elements.mobileNav.offsetHeight);
        }, false);

        // Click
        // Add daily / weekly tasks
        if (this.elements.weeklyTaskCTA) {
            this.elements.weeklyTaskCTA.addEventListener("click", function (event) {
                event.preventDefault();

                _this.addPeriodicTask(_this.elements.weeklyTaskCTA, "weekly");
            }, false);
        }

        if (this.elements.dailyTaskCTA) {
            this.elements.dailyTaskCTA.addEventListener("click", function (event) {
                event.preventDefault();

                _this.addPeriodicTask(_this.elements.dailyTaskCTA, "daily");
            }, false);
        }

        // Change feedback radio
        var _iteratorNormalCompletion = true;
        var _didIteratorError = false;
        var _iteratorError = undefined;

        try {
            for (var _iterator = this.elements.feedbackInputs[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                var item = _step.value;

                item.addEventListener("change", function (event) {
                    _this.feedbackSelectedItem = event.target;

                    _this.selectFeedbackItem(event);
                });
            }
        ...
        if (this.elements.feedbackSubmitBtn) {
            this.elements.feedbackSubmitBtn.addEventListener("click", function (event) {
                event.preventDefault();

                var feedbackText = _this.elements.feedbackTextbox.value;

                if (feedbackText.length) {
                    var request = $requestJSON("/guide/action/feedback");

                    _this.elements.feedbackSubmitBtn.innerText = "";
                    $add(_this.elements.feedbackSubmitBtn, DIV({ 'c': 'spinner_loader' }, Indicator.cssSpinner()));

                    $removeClass(_this.elements.feedbackCommentNotification, "td-guide__notification_message--error");
                    _this.elements.feedbackCommentNotification.innerText = "";

                    request.addErrback(function (e) {
                        $addClass(_this.elements.feedbackCommentNotification, "td-guide__notification_message--error");
                        _this.elements.feedbackCommentNotification.innerText = _this.messages.error;
                        _this.elements.feedbackSubmitBtn.innerText = "Submit";
                    });

                    request.addCallback(function (e) {
                        $addClass(_this.elements.feedbackWrapper, "td-guide__feedback--positive");
                        $removeClass(_this.elements.feedbackWrapper, "td-guide__feedback--negative");
                    });

                    request.sendReq({ text: feedbackText, guide: window.location.href });
                }
            }, false);
        }
    }

    // Get the window scroll postion
    // https://gist.github.com/SteAllan/8536274


    _createClass(GuideTOC, [{
        key: 'getScrollTop',
        ...
        value: function isElementInViewport(el) {
            ...
            var margin = this.dimensions.windowHeight / 3;

            return rect.top <= 0 + margin && rect.bottom > 0 + margin;
        }
        ...
        key: 'doWhenScrolling',
        value: function doWhenScrolling() {
            this.getScrollPosition();

            if (this.dimensions.windowWidth >= this.dimensions.minDesktopWidth) {
                this.updatePosition();
                this.updateActiveTocElement();
                this.getGuidePosition();
                this.updateProgress();
            } else {
                this.updateMobileNavPosition();
        ...
        key: 'doWhenResizing',
        value: function doWhenResizing() {
            this.getTocPosition();
            this.getWindowDimensions();
            this.getGuideHeight();
            this.getGuidePosition();

            this.doWhenScrolling();
        }
        ...
        key: 'getTocPosition',
        value: function getTocPosition() {
            this.dimensions.tocPos = $position(this.elements.toc).y;
        }
        ...
        key: 'getGuideHeight',
        value: function getGuideHeight() {
            this.dimensions.guideHeight = this.elements.guide.offsetHeight;
        }

        // Get guide position

    }, {
        key: 'getGuidePosition',
        value: function getGuidePosition() {
            this.dimensions.guidePosition = this.elements.guide.getBoundingClientRect();
        }
        ...
        value: function getScrollPosition() {
            this.dimensions.scrollPos = this.getScrollTop();
        }

        // Update position of header element

    }, {
        key: 'updatePosition',
        value: function updatePosition() {
            if (this.dimensions.guideHeight > (this.dimensions.guidePosition.top - this.dimensions.windowHeight) * -1) {
                $removeClass(this.elements.tocInner, "td-guide__sidebar-inner--pinned");

                if (this.dimensions.scrollPos >= this.dimensions.tocPos) {
                    $addClass(this.elements.tocInner, "td-guide__sidebar-inner--fixed");
                    return;
                }

                $removeClass(this.elements.tocInner, "td-guide__sidebar-inner--fixed");
            } else {
                $removeClass(this.elements.tocInner, "td-guide__sidebar-inner--fixed");
                $addClass(this.elements.tocInner, "td-guide__sidebar-inner--pinned");
            }
        }

        // Update visible elements on viewport

    }, {
        key: 'updateActiveTocElement',
        value: function updateActiveTocElement() {
            var _iteratorNormalCompletion2 = true;
            var _didIteratorError2 = false;
            var _iteratorError2 = undefined;

            try {
                for (var _iterator2 = this.tocIds[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                    var idItem = _step2.value;

                    var idEl = $$(idItem);

                    if (idEl.length) {
                        var tocItemLink = $$(".td-guide__toc-link[href='" + idItem + "']")[0];
                        var tocItemLi = $parent(tocItemLink, 'li');

                        if (this.isElementInViewport(idEl[0])) {
                            $addClass(tocItemLi, "td-guide__toc-item--active");
                        } else {
                            $removeClass(tocItemLi, "td-guide__toc-item--active");
                        }
                    }
                }
            } catch (err) {
        ...
        value: function updateProgress() {
            var curProgress = this.dimensions.guidePosition.top - Math.round(this.dimensions.windowHeight / 3) - 20;
            var curProgressPerc = curProgress * 100 / this.dimensions.guideHeight * -1;

            if (curProgressPerc < 0 || curProgressPerc === Infinity) {
                curProgressPerc = 0;
            } else if (curProgressPerc > 100) {
                curProgressPerc = 100;
            }
            ...
            try {
                for (var _iterator3 = this.elements.progressBars[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                    var bar = _step3.value;

                    bar.setAttribute("value", curProgressPerc);
                }
            } catch (err) {
        ...
        key: 'getTocIds',
        value: function getTocIds() {
            var _iteratorNormalCompletion4 = true;
            var _didIteratorError4 = false;
            var _iteratorError4 = undefined;

            try {
                for (var _iterator4 = this.elements.tocItems[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
                    var el = _step4.value;

                    this.tocIds.push(el.getAttribute("href"));
                }
            } catch (err) {
        ...
        key: 'throttle',
        value: function throttle(callback, fps) {
            var _this2 = this;
        ...
        key: 'goToSection',
        value: function goToSection(event, target) {
            var filterRegex = new RegExp("^\\s+", "g");

            if (target.selectedOptions) {
                this.elements.mobileNavTitle.innerText = target.selectedOptions[0].text.replace(filterRegex, "");
            }
        ...
        key: 'updateMobileNav',
        value: function updateMobileNav() {
            var options = this.elements.mobileNavTree.options;

            for (var i = 0; i < options.length; i++) {
                var option = options[i];
                var optionEl = $$("#" + option.value)[0];

                if (this.isElementInViewport(optionEl)) {
                    option.selected = true;
                    this.goToSection(null, this.elements.mobileNavTree);
                }
            }
        }

        // Update mobile nav position

    }, {
        key: 'updateMobileNavPosition',
        value: function updateMobileNavPosition() {
            if (this.dimensions.guidePosition.top - this.dimensions.windowHeight / 3 < 0 && this.dimensions.guidePosition.bottom - this.dimensions.windowHeight / 3 > 0) {
                $addClass(this.elements.mobileNav, "td-guide__mobile-nav--fixed");
            } else {
                $removeClass(this.elements.mobileNav, "td-guide__mobile-nav--fixed");
            }
        }
        ...
        key: 'addPeriodicTask',
        value: function addPeriodicTask(button, period) {
            var _this3 = this;

            if (!button.attributes['disabled']) {
                var request = $requestJSON("/guide/action/add_task");
                var notificationEl = this.elements[period + "TaskNotification"];

                $addClass(button, "td-guide__hidden-el");
                $addBefore(DIV({ 'c': 'spinner_loader' }, Indicator.cssSpinner()), button);

                $removeClass(notificationEl, "td-guide__notification_message--error");
                notificationEl.innerText = "";

                request.addErrback(function (e) {
                    $addClass(notificationEl, "td-guide__notification_message--error");
                    notificationEl.innerText = _this3.messages.error;
                    $removeClass(button, "td-guide__hidden-el");
                    $remove($$(".spinner_loader", button.parentNode));
                });

                request.addCallback(function (e) {
                    button.setAttribute("disabled", true);
                    button.innerText = _this3.messages.taskAdded;
                    $removeClass(button, "td-guide__hidden-el");
                    $remove($$(".spinner_loader", button.parentNode));
                });

                request.sendReq({ period: period });
            }
        }
        ...
        key: 'selectFeedbackItem',
        value: function selectFeedbackItem(event) {
            var _this4 = this;

            var voteValue = event.target.value == 1 ? "no" : "yes";
            var button = event.target.parentNode.childNodes[1];
            var request = $requestJSON("/guide/action/vote");

            $add(button, DIV({ 'c': 'spinner_loader' }, Indicator.cssSpinner()));

            $addClass(button, "td-guide__feedback-rating--is-loading");
            $removeClass(this.elements.feedbackNotification, "td-guide__notification_message--error");
            this.elements.feedbackNotification.innerText = "";

            request.addErrback(function (e) {
                $removeClass(button, "td-guide__feedback-rating--is-loading");
                $addClass(_this4.elements.feedbackNotification, "td-guide__notification_message--error");
                _this4.elements.feedbackNotification.innerText = _this4.messages.error;
                $remove($$(".spinner_loader", button.parentNode));
            });

            request.addCallback(function (e) {
                if (event.target.value == 1) {
                    $addClass(_this4.elements.feedbackWrapper, "td-guide__feedback--negative");
                } else {
                    $addClass(_this4.elements.feedbackWrapper, "td-guide__feedback--positive");
                }
            });

            request.sendReq({ yesno: voteValue, guide: window.location.href });
        }

        // Check if guide is minimal version

    }, {
        key: 'guideIsMinimal',
        value: function guideIsMinimal(guide) {
            return $hasClass(guide, 'td-guide--minimal');
        }

        // Unwrapp external links

    }, {
        key: 'unwrappLinks',
        value: function unwrappLinks(links) {
            ...
            try {
                for (var _iterator5 = links[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
                    ...
                    var linkedText = $strip(link.innerText);
                    var linkURL = link.getAttribute('href');
                    var externalURLRegex = new RegExp(/^http(?:s)?:\/\//, 'i');

                    if ($hasClass(link, 'td-guide__cta')) {
                        if (linkURL.match(externalURLRegex)) {
                            $addClass(link, 'td-guide__hidden-in-minimal');
                        }
                    } else if (linkedText && linkURL.match(externalURLRegex)) {
                        link.replaceWith(linkedText);
                    }
                }
            } catch (err) {
...
exports.default = GuideTOC;

},{"./.es6.Zoomable.js":1,"babel-polyfill":3}],3:[function(require,module,exports){
(function (global){
"use strict";
...
},{"core-js/fn/regexp/escape":4,"core-js/shim":327,"regenerator-runtime/runtime":328}],4:[function(require,module,exports){
